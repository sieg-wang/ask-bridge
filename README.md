...
此模式會開啟新的所選 provider 分頁，並清理先前同一 provider 的分頁。確切的處置範圍是：

- **會關閉**：同一個 provider 且網址讀得出來的分頁、空白分頁，以及停在該
  provider 登入網域的分頁（session 過期後 `--new` 自己留下的殘骸）。不關閉
  這三類，分頁會隨著每次執行累積。
- **會保留**：其他 provider 的分頁、其他網站的分頁，以及網址讀不回來、
  無法確認歸屬的分頁——叫不出名字的分頁沒有被證明是我們的。

若無法唯一辨識這次新開的分頁，命令會停止，且不會改用或關閉任何既有分頁。
